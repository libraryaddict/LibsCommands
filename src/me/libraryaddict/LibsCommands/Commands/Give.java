    ...
    private int getMaxAllowed(CommandSender sender, int itemId) {
        for (PermissionAttachmentInfo permissions : sender.getEffectivePermissions()) {
            if (permissions.getPermission().startsWith("bukkit.command.give")) {
                ...
                String[] strings = permissions.getPermission().replace("bukkit.command.give", "").split(".");
                if (strings.length > 0) {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        if (sender.hasPermission("bukkit.command.give")) {
            if (args.length > 0) {
                ...
                int[] returns;
                try {
                    returns = parseString(StringUtils.join(args, " ").substring(args[0].length() + 1).split(" "));
                ...
                if (p == null) {
                    sender.sendMessage(ChatColor.RED + "Error: Player '" + args[0] + "' cannot be found");
                    return true;
                }
